...
public class FollowerRecommendation  {
   public static void main(String[] args) throws Exception {
       ...
       ControlledJob step1 = new ControlledJob(jobListFollowers(args[0], intermediateFileDir), null);
       ControlledJob step2 = new ControlledJob(jobRecommendFollowers(intermediateFileDirFile, args[1]), Arrays.asList(step1));
...
/* The code above is the driver for the chained MapReduce jobs. The file output path for MapReduce job 1 must match exactly with the file input path for MapReduce job 2. Another critical issue is that we need to specify the dependency in "ControlledJob step2" so that MapReduce job 2 will only start to run after the first MapReduce job completes. */
   ...
   private static Job jobListFollowers(String inputPath, String outputPath) throws IOException, InterruptedException, ClassNotFoundException{      
       Job job = new Job();
...
/* The code above setup all the configuration for MapReduce job 1. */
   ...
   private static Job jobRecommendFollowers(String inputPath, String outputPath) throws IOException, InterruptedException, ClassNotFoundException{     
       Job job1 = new Job();
